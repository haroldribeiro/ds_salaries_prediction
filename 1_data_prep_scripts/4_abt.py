...
df['vaga_simplificada'] = np.where(df.vaga.str.contains('Analyst')
                          ...
                          np.where(df.vaga.str.contains('Director')
                                   , 'Director',
                          np.where(df.vaga.str.contains('Scien')
                                   , 'Data Scientist'
                                   , 'Unknown'))))))
...
df = (
    df.assign(
        salario_min=lambda x:
        x.salario_estimado.str.replace('[a-zA-Z(.):$]', '').str.strip().str.split('-', expand=True)[0].astype(int),
        salario_tmp=lambda x:
        x.salario_estimado.str.replace('[a-zA-Z(.):$]', '').str.strip().str.split('-', expand=True)[1],
...
#df.groupby('setor')['salario_medio'].mean('salario_medio').sort_values()
...
df['estatistica'] = np.where(df.descricao.str.lower().str.contains('math') |
                             ...
                             df.descricao.str.lower().str.contains('modeling')                              
                             , 1, 0)
...
df_estados = pd.read_csv(path + "states.csv", names=['estado', 'abv', 'codigo'], usecols=['estado', 'codigo'],
                         skiprows=1)
...
df_codigo_estados = df_estados_na[['cidade', 'codigo']].merge(df[['cidade', 'estado']], how='outer', left_index=True,
...
df['tamanho'] = df.tamanho.apply(lambda x: 'Unknown' if (x == '-1' or x.lower() == 'unknown') else x)
...
df = (
    df.assign(
        ...
        tamanho_num_tmp=lambda x: np.where(x.tamanho_tmp == 'Unknown', -1,x.tamanho_tmp.str.replace('[a-zA-Z\\+]', '').str.strip()),
...
# ---------------------------------------#
#       Foundation Year (Ano Fundacao)   #
# ---------------------------------------#

# checking size distribution. Large number of observations without foundation year (285)
...
# Checking the descriptive metrics
df.idade_empresa.describe()

# checking descriptive metrics without missing values
# mean > median and because of that we are gonna use median as interpolation technique
df.idade_empresa[df.idade_empresa != -1].describe()
# Applying median value
df['idade_empresa'] = np.where(df.idade_empresa == -1, df[df.idade_empresa != -1].idade_empresa.median(), df.idade_empresa)
...
df['tipo_propriedade'] = df.tipo_propriedade.apply(lambda x: 'Unknown' if x == '-1' else x)
...
# Adjusting missing value as constant
df['industria'] = df.industria.apply(lambda x: 'Unknown' if x == '-1' else x)
...
# Adjusting missing value as constant
df['setor'] = df.setor.apply(lambda x: 'Unknown' if x == '-1' else x)
...
df['receita'] = df.receita.apply(
    lambda x: 'Unknown' if (x.strip() == '-1' or x.lower().strip() == 'unknown / non-applicable') else x)

# extract revenue  as number
df = (
    df.assign(
        multiplicador=np.where(df.receita == 'Unknown', -1,
        ...
        receita_re=lambda x: np.where(x.receita_tmp == 'Unknown', '1', x.receita_tmp.str.replace('[a-zA-Z$\s()\\+]+', '').str.strip()),
        receita_num=lambda x: np.where(x.receita_tmp == 'Unknown', -1, x.receita_re.astype(int) * x.multiplicador)
...
colunas = ['vaga_simplificada', 'senioridade', 'nome_empresa', 'tamanho', 'tipo_propriedade', 'industria', 'setor',
           ...
           'descricao_qtd_palavras', #'tamanho_num', 'receita_num',
